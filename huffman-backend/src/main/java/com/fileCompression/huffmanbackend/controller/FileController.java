...
public ResponseEntity<Map<String, String>> decompressFile(@RequestBody MultipartFile file) {
    try {
        // Check if the file is not empty
        if (file.isEmpty()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        ...
        HuffmanTree huffmanTree = new HuffmanTree();
        HuffmanNode root = huffmanTree.buildHuffmanTree(originalPath);
        ...
        String decompressedFilePath  = saveToFile(decompressedData, "decompressed_file.txt");
        ...
        e.printStackTrace();
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(Map.of("error", "Error decompressing file: " + e.getMessage()));
    }
}
